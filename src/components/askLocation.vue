<template>
  <div class="bg-gray-900 h-full border border-gray-200 rounded-md p-4">
    <div class="text-gray-200 text-center text-2xl mb-4 font-bold">
      Select Location
    </div>
    <div v-if="loading">
      <v-loading></v-loading>
    ...
    <div v-else>
      <button
        v-if="nearestCity && nearestCity.location_name"
      ...
      <div
        v-if="!(nearestCity && nearestCity.location_name)"
        class="flex text-red-900 bg-red-200 p-2 justify-center rounded-md font-bold"
      >
  ...
  async created() {
    ...
    try {
      ...
      if (data) {
        ...
        let cityDistances = this.findNearestLocation(
    ...
    findNearestLocation(latitude, longitude) {
      return this.locations.map((item) => {
        ...
        let distance = this.calcCrow(
          parseFloat(cityCoordinates[0].trim()),
          parseFloat(cityCoordinates[1].trim()),
          latitude,
          longitude
        );
