  ...
  >
    <div class="flex flex-col justify-between">
      <div class="flex lg:justify-between flex-col lg:flex-row items-center">
        <h2 class="text-2xl text-yellow-400 font-bold leading-tight mb-4">
          {{ categoryName }}
        </h2>
        <v-input
          placeholder="Search"
          prefixIcon="search"
          size="small"
          class="lg:w-1/4 w-full mx-2"
          v-model="categorySearchString"
          @change="getBusinessListByCategory"
        ></v-input>
      </div>
      ...
      if (this.categoryId && this.location_id) {
        ...
        try {
          this.timeout = setTimeout(async () => {
            let response = await apiService.get("/businessByCategory", {
              search: this.categorySearchString,
