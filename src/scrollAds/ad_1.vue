<template>
    <div class="w-full">
				<div class="flex-row m-auto w-full bg-red-300 justify-center items-center border border-gray-200 p-2 flex ">
					<div class="h-40 md:h-60 lg:h-80">
					<announce-icon></announce-icon>
					</div>
					<div class="flex flex-col">
					<div class="text-center flex flex-col flex-grow">
						<div>
						<h1 class="text-xl md:text-2xl lg:text-4xl  text-red-800 font-bold">Show Your Ad Here</h1>
						<p class="mx-auto mt-2 md:text-sm text-xs lg:text-md text-gray-800">Contact Us to display your business advertisement.</p>
						</div>
						<router-link to="/about">
						<button class="mt-2 rounded-md bg-transparent px-8 py-4 font-medium  transition-colors bg-red-800 text-gray-200 disabled:opacity-50">Contact Us</button>
						</router-link>
					</div>
					</div>
				</div>
			</div>
</template>
<script>

import AnnounceIcon from '@/assets/illustrations/announceIcon.vue'
export default {
  name:"ad_1",
  components:{
    AnnounceIcon
  }
}
</script>